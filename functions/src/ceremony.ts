...
export const completeCeremony = functions.https.onCall(async (request: any) => {
    ...
    if (!gridKey || typeof gridKey !== 'string') {
        throw new functions.https.HttpsError(
            'invalid-argument',
            'Must provide a valid "gridKey" argument.'
        );
    }
    ...
    try {
        ...
        if (!ceremonyDoc.exists) {
            throw new functions.https.HttpsError(
                'not-found',
                'No active ceremony found for this location.'
            );
        }
        ...
        if (ceremony.ownerId !== uid) {
            throw new functions.https.HttpsError(
                'permission-denied',
                'Only the ceremony owner can complete the promotion.'
            );
        }
        ...
        if (ceremony.status !== 'active') {
            throw new functions.https.HttpsError(
                'failed-precondition',
                'This ceremony is no longer active.'
            );
        }

        // 4. Verify 9+ affirmations (owner is auto-counted as #1, so 9 others = 10 total)
        const affirmations: string[] = ceremony.affirmations || [];
        if (affirmations.length < 9) {
            throw new functions.https.HttpsError(
                'failed-precondition',
                `Need ${9 - affirmations.length} more affirmations (${affirmations.length}/9).`
            );
        }
        ...
        if (getRankValue(newRank) <= getRankValue(currentRank)) {
            throw new functions.https.HttpsError(
                'failed-precondition',
                `Cannot promote from ${currentRank} to ${newRank}.`
            );
        }
        ...
        throw new functions.https.HttpsError(
            'internal',
            'Failed to complete the ceremony.',
            error
        );
    }
});
