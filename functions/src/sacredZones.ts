...
export const seedSacredZones = functions.https.onCall(async (request: any) => {
    ...
    try {
        ...
        for (const el of elements) {
            ...
            if (zoneCount >= 400) break;

            const name = el.tags?.name || el.tags?.religion || 'Sacred Space';
            // We use '🙏' universally
            const icon = '🙏';
            ...
            let boundary: { lat: number; lng: number }[] | undefined;
            ...
            if (el.type === 'node') {
                ...
                type = 'polygon';
                boundary = el.geometry.map((g: any) => ({ lat: g.lat, lng: g.lon }));
                ...
                if (outerMembers.length > 0) {
                    ...
                    for (const m of outerMembers) {
                        boundary.push(...m.geometry.map((g: any) => ({ lat: g.lat, lng: g.lon })));
                    }
                }
            }
